...
//                  formato pre-impreso.
// HPC_201707_ASO : Referencia en Previsión social HPC_201706_PRE, Se pasa fuente y formulario del modulo de previsión social (PRE295 y Fnuerepcueind)
// ASO_201807_HPC : Referencia en Previsión social HPC_201812_PRE, Se pasa fuente y formulario del modulo de previsión social (PRE295 y Fnuerepcueind)
Unit PRE295;
Interface
...
Begin
   ...
   Begin
      ...
      Begin
         ...
         xSqlctaind := xSqlctaind +
         // Inicio: ASO_201807_HPC
         // Se modifica Qry con la finalidad de corregir error en el orden de los meses
         ...
         ' SELECT ROWNUM+2 ORDEN, DESCRIPCION, INTERES, BONUS, APORTE, SALDO FROM (SELECT ANO||MES DESCRIPCION, NVL(INTCAPACU,0) INTERES, NVL(BONOS,0) BONUS, NVL(CAPITAL,0) APORTE, NVL(ACUMULADO,0) SALDO FROM T_CTAIND1'
         +' WHERE ASOID = '+QuotedStr(DM1.cdsAsociado.FieldByName('ASOID').AsString)+' AND ANO||MES > '+QuotedStr(xAnoMesIni)+' AND ANO||MES <= '+QuotedStr(xanomesces)
         +' ORDER BY ANO||MES)';
         // Fin: ASO_201807_HPC
         DM1.cdsQry21.Close;
      ...
      Begin
         ...
         xSqlctaind := xSqlctaind +
         // Inicio: ASO_201807_HPC
         // Se modifica el valor de los ultimos 6 meses de aportes
         // 'SELECT ROWNUM ORDEN, ANO||MES DESCRPCION, NVL(INTCAPACU,0) INTERES, NVL(BONOS,0) BONUS, NVL(CAPITAL,0) APORTE, NVL(ACUMULADO,0) SALDO FROM T_CTAIND1'
         //   + ' WHERE ASOID = '+QuotedStr(DM1.cdsAsociado.FieldByName('ASOID').AsString)+' AND ANO||MES > ' + QuotedStr(xAnoMesIni)+ ' AND ANO||MES <= '+QuotedStr(xanomesces);
         ' SELECT ROWNUM+2 ORDEN, DESCRIPCION, INTERES, BONUS, APORTE, SALDO FROM (SELECT ANO||MES DESCRIPCION, NVL(INTCAPACU,0) INTERES, NVL(BONOS,0) BONUS, NVL(CAPITAL,0) APORTE, NVL(ACUMULADO,0) SALDO FROM T_CTAIND1'
         +' WHERE ASOID = '+QuotedStr(DM1.cdsAsociado.FieldByName('ASOID').AsString)+' AND ANO||MES > '+QuotedStr(xAnoMesIni)+' AND ANO||MES <= '+QuotedStr(xanomesces)
