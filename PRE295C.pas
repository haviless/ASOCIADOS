...
// Objetivo             : Formulario de Reportes de cuentas individuales para asociados activos
// Actualizaciones      :
//HPC_201702_ASO        : Creación del Formulario

//Inicio HPC_201702_ASO
//Creación de formulario Reportes de cuentas individuales para asociados activos
// HPC_201706_ASO : Se muestra numero de DNI en el campo DNI del reporte detallado de cuenta individual      
...
Begin
   ...
   xSql := 'SELECT MAX(PVSEXPNRO) PVSEXPNRO FROM PVS301 A WHERE A.ASOID = '+QuotedStr(DM1.cdsAsociado.FieldByName('ASOID').AsString)+' AND PVSESTADO = ''01''';
   DM1.cdsQry.Close;
   DM1.cdsQry.DataRequest(xSql);
   DM1.cdsQry.Open;
   If DM1.cdsQry.FieldByName('PVSEXPNRO').AsString <> '' Then xtipcueind := '2';
   ...
   Begin
      ...
      Begin
         ...
         Begin
            ...
            Begin
              DM1.cdsCueInd.FieldByName('ANO').AsString := Copy(DM1.NombreMes(Copy(DM1.cdsAsociado.FieldByName('ASOFRESNOM').AsString,4,2)),1,3)
                                                           +' '+Copy(DM1.cdsAsociado.FieldByName('ASOFRESNOM').AsString,7,4)+' A DIC 1996';
...
Begin
   ...
   Begin
      ...
      If xtipcueind = '3' Then
         DM1.tTiempoTranscurrido := DM1.tiempoTranscurridoPRE(dm1.FechaSys, StrToDate(xasofresnom), '0')
      Else
         DM1.tTiempoTranscurrido := DM1.tiempoTranscurridoPRE(StrToDate( xasofrescese), StrToDate(xasofresnom) , '0');
      ...
      Begin
         ...
         pplliquidacioncanceladares.Caption := 'CUENTA INDIVIDUAL CANCELADA. LIQUIDACIÓN Nº '+Copy(xpvslbennr,1,4)+'-'+Copy(xpvslbennr,5,7);
         pplliquidacioncanceladadet.Visible := True;
         pplliquidacioncanceladadet.Caption := 'CUENTA INDIVIDUAL CANCELADA. LIQUIDACIÓN Nº '+Copy(xpvslbennr,1,4)+'-'+Copy(xpvslbennr,5,7);
         pplliquidacioncanceladaest.Visible := True;
         pplliquidacioncanceladaest.Caption := 'CUENTA INDIVIDUAL CANCELADA. LIQUIDACIÓN Nº '+Copy(xpvslbennr,1,4)+'-'+Copy(xpvslbennr,5,7);
      End;
      ...
      Begin
         ...
         // Inicio: HPC_201706_ASO
         //  Se carga del valor del DNI del asociado en la variable pplblasodni
